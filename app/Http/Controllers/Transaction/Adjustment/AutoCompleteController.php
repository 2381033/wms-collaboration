...
class AutoCompleteController extends Controller
{
        ...
        if ($request->ajax()) {
            $stock = DB::table("iv_stock_ledger as a")
                        ->select(
                            ...
                            DB::raw("a.qtya % d.uppp % d.muppp as bqty"), "d.puom", "d.muom", "d.buom", "a.lot_no", "a.exp_date", "a.mfg_date",
                            "a.product_code", "d.product_name", "e.site_name", "f.area_name", "a.location_code", "a.id"
                        )
    ...
    public function stockEdit(Request $request)
    {
        $list = DB::table("iv_stock_ledger as a")
                    ->select(
                        "a.id", "a.product_id", "c.product_name", "a.lot_no", "a.po_number", "a.mfg_date", "a.exp_date",
                        ...
                        DB::raw("a.qtya % c.uppp % c.muppp as bqty"), "c.puom", "c.muom", "c.buom",
                        "c.uppp", "c.muppp", "a.site_id", "d.site_name", "a.area_id", "e.area_name", "a.location_id", "a.location_code",
                        "a.principal_id", "b.principal_name", "a.manufactur_id", "f.manufactur_name", "a.status_id", "g.status_name", "c.unit_level"
                    )
