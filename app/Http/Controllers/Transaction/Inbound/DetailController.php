...
class DetailController extends Controller
{
    ...
    {
        $data = inboundDetails::from('iv_inbound_detail as a')
    ...
    public function store(Request $request)
    {
        ...
        $exception = DB::transaction(function () use ($request) {
            ...
            try {
                ...
                $detail->qrcode = Str::random(30);
                $job->allocated_flag = 'No';
                $job->allocated_by = null;
    ...
    {
        try {
            $detail = inboundDetails::where('id', $request->id)->first();

            $detail_all = inboundDetails::where('vehicle_no', $detail->vehicle_no)
                ->where('id', '<>', $detail->id)
                ->get();
    ...
    public function import(Request $request)
    {
        ...
        $import = new InboundPackingImports();
        ...
        foreach ($rows[0] as $row) {
            ...
            if ($vehicle > 0 && isset($product) && $detail == 0) {
                ...
                if ($error_flag == false) {
                    if ($job->principal_id == 32) {
                        $qty_1 = $qty_1 / $product->muppp;
                        $qty_2 = 0;
                        // dd(is_int($qty_1));
                        if (is_int($qty_1)) {
                            $qty_1 = $qty_1;
                            ...
                            $qty_ex = explode(".", $qty_1);
                            $qty_1 = (int)$qty_ex[0];
                            $qty_2 = (int)$qty_ex[1] + 1;
                        }
                    }
                    $insert[] = [
    ...
    public function detailPallet($picking_id)
    {
    ...
    public function getScanLokasi($location_code)
    {
        $mySite = DB::table('users_site')->where('user_id', Auth::user()->id)->first()->site_id;
        ...
        $exception = DB::transaction(function () use ($request) {
            try {
                $detail = DB::table('iv_location')
                    ...
                    ->first();
                if ($detail->site_id == 3) {  //validasi double location khusus belawan
                    $ledger = stockLedger::where('location_id', $detail->id)
                        ->where('qtys', '>', 0)
                        ->count();
                    if ($ledger > 0 && $detail->status_code != 'B') { //khusus area yang bukan damage
                        $message =  'stock_exist';
                        DB::rollBack();
                    } else {
                        DB::table('iv_inbound_per_pallet')
