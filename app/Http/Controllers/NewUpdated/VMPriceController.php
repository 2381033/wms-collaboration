...
class VMPriceController extends Controller
{
    private function objectService($service)
    {
        $data = DB::table('price_service')
            ->where('id', $service)
            ->first();
    ...
    private function getHistoryMaster($id)
    {
        $data = DB::table('price_history')
            ->orderBy('id', 'DESC')
            ->where('master_id', $id)
            ->get();
    ...
    public function getMOT($service)
    {
        $data = DB::table('price_master')
            ->select('mot')
    ...
    public function getVendor($service, $mot)
    {
        $data = DB::table('price_master')
            ->select('vendor')
            ->where('service', $service)
    ...
    private function getMasterPrice($service, $mot)
    {
        $data = DB::table('price_master')
            ->where('service', $service)
            ->where('mot', $mot)
