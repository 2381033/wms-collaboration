...
class SiteIndicatorController extends Controller
{
    public function index(Request $request) {
        $company_id = Auth::user()->company_id;

        $details = MasterSiteIndicator::where('company_id', $company_id)
        ...
        $type = MasterSiteType::where('active', 'Yes')->get();

        $data = [
            'type_list' => $type
        ];

        return view('master.site-indicator', $data);
    }

    public function store(Request $request)
    {
        $messsages = array(
            'indicator_name.required'=>'Indicator name cannot be empty.',
        );
        ...
        MasterSiteIndicator::updateOrCreate(['id' => $id],
        [
    ...
    {
        $data  = MasterSiteIndicator::where('id', $id)->first();
    ...
    {
        try {
            MasterSiteIndicator::where('id',$request->id)->delete();
