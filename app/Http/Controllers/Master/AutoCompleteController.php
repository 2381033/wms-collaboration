...
class AutoCompleteController extends Controller
{
    ...
    public function getProduct(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $search_text = (is_null($search) || empty($search) || strlen($search) < 1)
                ? "%"
                : "%" . $search . "%";
            ...
            $useAlias = ($typeProduct && $typeProduct->inbound_alias_code === "true");

            $list = DB::table('iv_product as a')
                ->select(
                    "a.id",
                    DB::raw($useAlias ? "a.alias_code as product_code" : "a.product_code"),
                    "a.product_name",
                    "a.puom",
                    "a.muom",
                    "a.buom",
                ...
                ->where(function ($query) use ($search_text) {
                    $query->where("a.product_code", "LIKE", $search_text)
                        ->orWhere("a.alias_code", "LIKE", $search_text)
