...
class UoMController extends Controller
{
    public function index(Request $request) {
        ...
        return view('reference.uom');
    }

    public function store(Request $request)
    {
        $messsages = array(
            'code.required'=>'Unit code cannot be empty.',
            'uom_name.required'=>'Unit name cannot be empty.',
        );
        ...
        ReferenceUoM::updateOrCreate(['id' => $id],
        [
            'code'=>$request->code,
    ...
    {
        $where = array('id' => $id);
        $data  = ReferenceUoM::where($where)->first();
    ...
    {
        try {
            $data = ReferenceUoM::where('id',$request->id)->delete();
