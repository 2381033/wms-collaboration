...
class PickingReportExcel implements FromView
{
    ...
    public function view(): View
    {
        ...
        $transaction = DB::table('iv_stock_transaction')
            ->select('product_code', 'location_code', 'lot_no', 'job_type', 'qty', 'reference_no', 'created_at')
            ->orderBy('location_code', 'ASC')
            ->whereYear('job_date', date('Y'))
            ->whereIn('product_id', $productID)
            ->whereIn('location_id', $locationID)
            ->whereIn('lot_no', $lotNo)
            // ->where('product_code', 'PLAVA')
            // ->where('location_code', 'GG.011.J')
            // ->where('lot_no', '24T2403EU')
            ->get();

        // Group by product_code, location_code, and lot_no
        ...
        $results = $grouped->map(function ($items) use ($header) {
            ...
            $impQty = $items->where('job_type', 'IMP')->sum('qty');
            $expQty = $items->filter(function ($value) use ($confirmDate, $header) {
                return Carbon::parse($value->created_at) < $confirmDate &&
