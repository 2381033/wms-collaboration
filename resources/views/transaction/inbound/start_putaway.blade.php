...
@section('modal')
    <div class="modal fade" tabindex="-1" role="dialog" id="modal-submit-pallet-tag" data-backdrop="static"
        ...
        <div class="modal-dialog modal-dialog-centered modal-md" role="document">
            <form action="{{ route('inbound-scan_pallet_tag') }}" method="post" id="form-scan-pallet">
                @csrf
    ...
    <div class="modal fade" role="dialog" id="modal-edit-location">
        <div class="modal-dialog modal-lg" role="document">
            <form action="{{ route('inbound-edit_location_putaway') }}" method="post" id="form-scan-pallet">
                @csrf
                <div class="modal-content">
                                ...
                                <div class="form-group">
                                    <select id="locationCodeEdit" class="form-control" name="" style="width: 100%;"
                                        ...
                                        @foreach ($location as $item)
                                            <option
                                                value="{{ $item->status_code . ':' . $item->location_code . ':' . $item->id }}">
    ...
    <div class="modal fade" role="dialog" id="modal-submit-scan-lokasi">
        <div class="modal-dialog modal-lg" role="document">
            <form action="{{ route('inbound-scan_lokasi') }}" method="post" id="form-scan-lokasi">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-footer bg-whitesmoke br">
                        <button type="button" class="btn btn-info btn-sm" data-dismiss="modal"><i
                                class="fas fa-window-close"></i> <span>Close</span></button>
                        {{-- <button type="submit" class="btn btn-primary btn-sm btn-scan-lokasi hide"> Simpan</button> --}}
                    </div>
                </div>
            </form>
        </div>
    ...
    {{-- <div class="modal fade" role="dialog" id="modal-manual-lokasi">
        <div class="modal-dialog modal-lg" role="document">
            <form action="{{ route('inbound-scan_lokasi') }}" method="post" id="form-scan-lokasi">
                @csrf
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <select id="locationCode" class="form-control" name="" style="width: 100%;"
                                        onchange="damageArea(this.value)">
                                        @foreach ($location as $item)
                                            <option
                                                value="{{ $item->status_code . ':' . $item->location_code . ':' . $item->id }}">
                                                {{ $item->location_code }} - {{ $item->site_name }}
                                ...
                                <input type="hidden" name="location_code" id="locationCodeValue">
                                <input type="hidden" name="location_id" id="locationIDValue">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer bg-whitesmoke br">
...
@push('scripts')
    <script>
        ...
        function generateTablePutaway() {
            $.ajax({
                url: "{{ url('warehouse/inbound/startPutaway/getListPutaway') }}/" + '{{ $picking_id }}',
                type: "GET",
                dataType: 'json',
        ...
        $('#tableLocation').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ url('warehouse/inbound/startPutaway/getLocationAvail') }}/" + '{{ $inbound_id }}',
                type: "GET",
            },
            columns: [{
                    data: 'location',
        ...
        function doScanPallet() {
            ...
            $.ajax({
                url: "{{ url('warehouse/inbound/scanPalletTag') }}/" + qrcode + '/' + id_per_pallet + '/' +
                    product_code,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
        ...
        $('#form-scan-pallet').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('inbound-scan_pallet_tag') }}",
        ...
        function doScanLocation(paramsLocation) {
            $.ajax({
                url: "{{ url('warehouse/inbound/scanLokasi') }}/" + paramsLocation,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
