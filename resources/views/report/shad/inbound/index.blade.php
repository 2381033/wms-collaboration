...
@section('content')    
    ...
    <section id="contact" class="contact">
        <div class="container">
            <form id="form-filter" name="form-filter" action="{{route('shad-inbound.print')}}" method="post" onsubmit="target_popup(this)">
                @csrf
...
<script>        
    ...
    function downloadExcel() {
        ...
        var url = "{{URL::to('report/shad/inbound/export')}}?" + $.param(query)
        
        window.open(url, '_blank');
    }
    
    function target_popup(form) {
        window.open('', 'InboundReport', 'width=800,height=600,resizeable,scrollbars');
        form.target = 'InboundReport';
    }
</script>
@endpush