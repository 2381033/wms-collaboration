                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select name="principal_id" id="principal_id" class="form-control">
                                @foreach ($principal_list as $item)
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.principal', function () {
            ...
            $('#principal_table').DataTable({
                ...
                info : false,
                ajax : {
                    url : "{{ route('user-principal.index') }}",
                    type : "GET",
                    data : { 
                        user_id: dataId
                    } 
            ...
            $("#form-principal").validate({
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('user-principal.store') }}",
                        type: "POST", 
                        dataType: 'json',
                        success: function (data) { 
                            if($.isEmptyObject(data.error)){
